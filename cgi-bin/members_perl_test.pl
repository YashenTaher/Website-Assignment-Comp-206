...
sub check_valid
{
    ...
    foreach $entry (@entries)
	    ...
	    if (length($name) eq length($entry)) {my $lookfor="\(|\)|\[|\]|\\|\||\/|\*|\^|\{|\}|\$|\.|\+|\?|\,|\<|\>|\:|\;|\'|\"|\=|\-|\_|\&|\%|\#|\@|\!";}
	    else {my $lookfor="\(|\)|\[|\]|\\|\||\/|\*|\^|\{|\}|\$|\.|\+|\?|\,|\<|\>|\:|\;|\'|\"|\=|\-|\_|\&|\%|\#|\@|\!| ";}
...
print q(<meta http-equiv="refresh" content="5; url="http://cgi.cs.mcgill.ca/~ytaher1/home_page.html">);    ### change this link to the home page ##also this submits all forms on the page so homepage cannot contain a form!
print qq(\n </head> \n);
print qq(<body> \n);
